...
#define NS_DECL_NON_VIRTUAL_NSITASKBARTABPREVIEW \
  ...
  NS_METHOD GetIcon(imgIContainer * *aIcon); \
  NS_METHOD SetIcon(imgIContainer *aIcon); \
  NS_METHOD Move(nsITaskbarTabPreview *aNext); \
  NS_METHOD_(nativeWindow) GetHWND(void); \
  NS_METHOD_(void) EnsureRegistration(void); 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsTaskbarTabPreview, nsITaskbarTabPreview)
